...
def test_budget_limits_are_merged_at_240_per_hour():
    """Final integration: EXPLORATION budget is 240/h; window stays 3600s."""
    config = BudgetConfig()
    assert config.window_seconds == 3600
    assert config.max_calls_per_window == 240
    assert config.reserve_calls == 60
    assert config.general_pool == 180
