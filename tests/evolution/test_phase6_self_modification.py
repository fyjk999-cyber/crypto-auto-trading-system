...
def test_allowed_write_and_diff_budget():
    ...
    ok, _ = manager.record_write(ws, "src/crypto_trader/factors/weights.json", loc_added=5)
    assert ok is True
...
def test_validation_pipeline_all_gates_pass():
    run = ValidationPipeline().run(
        ...
        champion_metrics={"sharpe": 1.0},
        challenger_metrics={"sharpe": 1.2},
        success_metrics={"sharpe": "improve"},
        guardrail_metrics={},
    )
...
def test_validation_pipeline_guardrail_blocks_good_return():
    run = ValidationPipeline().run(
        ...
        champion_metrics={"return": 0.1},
        challenger_metrics={"return": 0.3},
