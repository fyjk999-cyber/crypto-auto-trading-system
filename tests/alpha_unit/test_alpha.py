...
def test_market_data_engine_realized_vol_insufficient_returns_none():
    mde = MarketDataEngine("BTCUSDT")
    ts = TS
    for _ in range(2):
        ts += timedelta(minutes=1)
...
def test_market_data_engine_sma_insufficient_returns_none():
...
def test_market_data_engine_zscore_insufficient_returns_none():
    mde = make_mde([Decimal("100"), Decimal("101")])
