...
def test_full_lifecycle_open_partial_fill_filled():
    ...
    assert (
        sm.transition(OrderStatus.CREATED, OrderEventType.ORDER_VALIDATED).new_status
        == OrderStatus.VALIDATED
    )
    assert (
        sm.transition(OrderStatus.VALIDATED, OrderEventType.ORDER_SUBMITTING).new_status
        == OrderStatus.SUBMITTING
    )
    assert (
        sm.transition(OrderStatus.SUBMITTING, OrderEventType.ORDER_SUBMITTED).new_status
        == OrderStatus.SUBMITTED
    )
    assert (
        sm.transition(OrderStatus.SUBMITTED, OrderEventType.ORDER_ACKNOWLEDGED).new_status
        == OrderStatus.ACKNOWLEDGED
    )
    assert (
        sm.transition(OrderStatus.ACKNOWLEDGED, OrderEventType.ORDER_OPENED).new_status
        == OrderStatus.OPEN
    )
    assert (
        sm.transition(OrderStatus.OPEN, OrderEventType.ORDER_PARTIALLY_FILLED).new_status
        == OrderStatus.PARTIALLY_FILLED
    )
    assert (
        sm.transition(OrderStatus.PARTIALLY_FILLED, OrderEventType.ORDER_FILLED).new_status
        == OrderStatus.FILLED
    )
...
def test_recovery_from_unknown():
    sm = OrderStateMachine
    assert (
        sm.transition(OrderStatus.UNKNOWN, OrderEventType.ORDER_OPENED).new_status
        == OrderStatus.OPEN
    )
    assert (
        sm.transition(OrderStatus.UNKNOWN, OrderEventType.ORDER_FILLED).new_status
        == OrderStatus.FILLED
    )
