...
def make_signal(qty="1"):
    return SignalIntent(signal_id="sig_1", strategy_id="test", symbol="BTCUSDT",
                        side=OrderSide.BUY, quantity=qty, limit_price="100")
...
def test_risk_approves_normal_order():
    engine = RiskEngine()
    decision = engine.check(make_signal(), account=make_account(), positions={},
                            market_price=Decimal("100"), open_order_count=0)
...
def test_max_order_notional_rejects():
    ...
    decision = engine.check(make_signal(qty="1"), account=make_account(), positions={},
                            market_price=Decimal("100"), open_order_count=0)
...
def test_max_open_orders_rejects():
    ...
    engine = RiskEngine(config)
    decision = engine.check(make_signal(), account=make_account(), positions={},
                            market_price=Decimal("100"), open_order_count=3)
...
def test_max_symbol_exposure_rejects():
    ...
    engine = RiskEngine(config)
    pos = Position(symbol="BTCUSDT", base_asset="BTC", quote_asset="USDT",
                   quantity=Decimal("1"), avg_entry_price=Decimal("80"), cost_basis=Decimal("80"))
    decision = engine.check(make_signal(qty="0.2"), account=make_account(), positions={"BTCUSDT": pos},
                            market_price=Decimal("100"), open_order_count=0)
...
def test_consecutive_failures_rejects():
    ...
    engine = RiskEngine(config)
    decision = engine.check(make_signal(), account=make_account(), positions={},
                            market_price=Decimal("100"), open_order_count=0, consecutive_failures=5)
