...
function CurrentPosition({ snapshot, compact = false }: { snapshot: TradingSnapshot; compact?: boolean }) {
  ...
  const market = record(snapshot.optional["/market"]?.data);
  return <div className={`current-position ${compact ? "compact" : ""}`}><div className="position-title"><strong>{position.symbol}</strong><span className={side.tone}>{side.code}</span><b>{numberText(Math.abs(Number(position.quantity)), 8)} {position.base_asset ?? position.symbol.replace(/USDT$/, "")}</b></div><dl><div><dt>入场价</dt><dd>{money(position.avg_entry_price)}</dd></div><div><dt>标记价格</dt><dd>{money(position.mark_price ?? pick(market, "mark_price", "price"))}</dd></div><div><dt>未实现盈亏</dt><dd>{money(position.unrealized_pnl)}</dd></div><div><dt>已实现盈亏</dt><dd>{money(position.realized_pnl)}</dd></div><div><dt>杠杆</dt><dd>{position.leverage === undefined ? "--" : `${numberText(position.leverage)}x`}</dd></div><div><dt>爆仓距离</dt><dd>--</dd></div></dl></div>;
}
  ...
  return <>
    ...
    <section className="trading-workspace">
      <Panel title="BTCUSDT 行情" className="market-panel" action={<span className={`source-badge ${source.toLowerCase()}`}>{sourceLabel}</span>}>
        <div className="market-source-header"><strong>BTCUSDT</strong><span>行情源：{klineProvider} · {klineSourceLabel}</span><span>执行：本地 PAPER 模拟器</span></div>
