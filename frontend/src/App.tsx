...
function CurrentPosition({ snapshot, compact = false }: { snapshot: TradingSnapshot; compact?: boolean }) {
  ...
  const mark = pick(position, "mark_price");
  const isPerpetual = position.market_type === "PERPETUAL";
  return <div className={`current-position ${compact ? "compact" : ""}`}><div className="position-title"><strong>{position.symbol}</strong><span className={side.tone}>{side.code}</span><b>{numberText(Math.abs(Number(position.quantity)), 8)} {position.base_asset ?? "BTC"}</b></div><dl><div><dt>入场价</dt><dd>{money(position.avg_entry_price)}</dd></div><div><dt>标记价格</dt><dd>{money(mark)}</dd></div><div><dt>未实现盈亏</dt><dd>{money(position.unrealized_pnl)}</dd></div><div><dt>已实现盈亏</dt><dd>{money(position.realized_pnl)}</dd></div><div><dt>杠杆</dt><dd>{position.leverage && position.leverage !== "NOT_APPLICABLE" ? `${numberText(position.leverage)}x` : "NOT_APPLICABLE"}</dd></div><div><dt>爆仓距离</dt><dd>{text(position.liquidation_price, "NOT_APPLICABLE")}</dd></div></dl></div>;
}
...
function PositionsPage({ snapshot }: { snapshot: TradingSnapshot }) {
  const rows = Object.values(snapshot.positions.data ?? {}).filter((item) => Number(item.quantity) !== 0);
  return <Panel title="持仓明细" source={snapshot.positions}>{snapshot.positions.status !== "ready" || !rows.length ? <EmptyBlock source={snapshot.positions} /> : <div className="table-wrap"><table><thead><tr><th>交易对</th><th>方向</th><th>数量</th><th>入场价</th><th>标记价格</th><th>未实现盈亏</th><th>已实现盈亏</th><th>杠杆</th><th>爆仓价</th></tr></thead><tbody>{rows.map((position) => { const side = positionDirection(position); return <tr key={position.symbol}><td><strong>{position.symbol}</strong></td><td><span className={side.tone}>{side.code}</span></td><td>{numberText(Math.abs(Number(position.quantity)), 8)}</td><td>{money(position.avg_entry_price)}</td><td>{money(position.mark_price)}</td><td>{money(position.unrealized_pnl)}</td><td>{money(position.realized_pnl)}</td><td>{position.leverage && position.leverage !== "NOT_APPLICABLE" ? `${numberText(position.leverage)}x` : "NOT_APPLICABLE"}</td><td>{text(position.liquidation_price, "NOT_APPLICABLE")}</td></tr>; })}</tbody></table></div>}</Panel>;
}
