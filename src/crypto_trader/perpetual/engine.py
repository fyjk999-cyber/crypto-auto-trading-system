...
class PerpetualPaperEngine:
    def __init__(
        ...
        contracts: list[PerpetualContract] | None = None,
    ) -> None:
        """Paper perpetual engine over a REGISTRY of contracts.

        ``contract`` is the canonical/primary contract (back-compat); the
        optional ``contracts`` list registers additional symbols so one
        engine instance serves every generic paper-perpetual contract.
        Positions remain keyed by execution symbol.
        """
        self.session_factory = session_factory
        self.contract = contract
        self.contracts: dict[str, PerpetualContract] = {contract.symbol: contract}
        for extra in contracts or []:
            self.contracts[extra.symbol] = extra
    ...
    def contract_for(self, symbol: str) -> PerpetualContract | None:
        """Contract for an execution symbol, or None if not registered."""
    ...
    async def open_position(
        ...
        contract = self.contracts.get(symbol) if symbol else None
        ...
        pos = self._cache.positions[symbol]
    ...
    async def close_position(
        ...
        if pos.side == PositionSide.LONG:
            realized = (exit_px - entry) * qty * contract.contract_size
        else:
            realized = (entry - exit_px) * qty * contract.contract_size
        margin_release = (
            pos.initial_margin * qty / abs(pos.quantity) if pos.quantity else Decimal("0")
        )
        fee = qty * exit_px * contract.contract_size * contract.taker_fee_rate
        ...
        await self.load_state()
        return self._cache.positions.get(symbol)

    async def apply_funding(
        self, rate: Decimal, mark_price: Decimal, symbol: str | None = None
    ) -> Decimal:
        contract = self.contracts.get(symbol) if symbol else None
        contract = contract or self.contract
        state = await self.load_state()
        pos = state.positions.get(contract.symbol)
        if pos is None or pos.is_flat:
            return Decimal("0")
        payment = self.funding.payment(pos, D(rate), D(mark_price), contract.contract_size)
        if payment.amount != 0:
            await self.futures_ledger.record_funding(
                contract, pos.side, payment.amount, payment.rate, payment.notional
            )
            await self.load_state()
        return payment.amount

    async def mark_to_market(
        self, mark_price: Decimal, symbol: str | None = None
    ) -> MarginPosition | None:
        contract = self.contracts.get(symbol) if symbol else None
        contract = contract or self.contract
        state = await self.load_state()
    ...
    async def liquidate(self, mark_price: Decimal, symbol: str | None = None) -> bool:
        contract = self.contracts.get(symbol) if symbol else None
        contract = contract or self.contract
        state = await self.load_state()
        pos = state.positions.get(contract.symbol)
        if pos is None:
            return False
        await self.mark_to_market(mark_price, contract.symbol)
        result = self.liquidation.evaluate(pos, contract, D(mark_price))
