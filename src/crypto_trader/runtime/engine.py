...
#: Bounded event retry: an event is retried a small fixed number of times,
...
class TradingEngine:
    ...
    async def start(self, run_id: str | None = None) -> str:
        ...
        # FILL SETTLEMENT RECOVERY: RecoveryService may have discovered factual
        # broker fills, and any earlier interruption (event failure, crash,
        # dropped event) leaves a durable fill without its ledger posting /
        # projection / plan convergence. Scanning AFTER RecoveryService catches
        # both. Fail closed: an unsettled fill must not let trading loops start
        # on an accounting state we cannot vouch for.
        try:
    ...
    async def _event_loop(self) -> None:
        while True:
            ...
            # Bounded retry: an event is no longer silently discarded. Replaying
            # is safe because fill application is idempotent on fill_id and
            # settlement completion is driven by the durable marker, so a retry
            # can only COMPLETE an interrupted settlement - never duplicate one.
            ...
            if last_exc is not None:
                ...
                try:
                    await self.audit.log(
                        "EXCHANGE_EVENT_PROCESSING_FAILED",
                        target=str(getattr(event, "event_id", "unknown")),
                        run_id=self.run_id,
                        ...
                        after={
                            ...
                            "exception_message": str(last_exc)[:500],
                            "attempt_count": MAX_EVENT_ATTEMPTS,
                        },
                    )
                except Exception:
                    logger.warning("event failure audit failed", exc_info=True)
    ...
    async def _reconciliation_loop(self) -> None:
        while True:
            ...
            # Bounded settlement recovery FIRST: normal reconciliation compares
            # projections against the exchange, which is meaningless while a
            # durable fill is still unaccounted.
            try:
                await self._recover_fill_settlements()
                settlement_ok = True
            except Exception as exc:
            ...
            # A settlement blocker must never be overwritten by a clean normal
            # reconciliation result.
            self.reconciliation_halted = report.halt or not settlement_ok
    ...
    async def _settle_fill(self, fill: Fill, *, record_metric: bool = True):
        ...
        if order is None:
            return None
        # Observability must count EXECUTIONS, not settlement passes. When this
        # runs as a recovery of an already-accounted fill the metric is skipped,
        # so a replay cannot inflate it.
        ...
        await self.audit.log(
            ...
            after={"transaction_id": str(getattr(settlement_txn, "transaction_id", fill.fill_id))},
        )
    ...
    async def _settlement_callback(self, fill) -> None:
        ...
        Records a durable failure reason before re-raising, so a dropped event
        leaves evidence of WHY the fill is unsettled. The event loop retries
        this; because settlement is idempotent, a retry can only complete it.
        """
        ...
        try:
            ...
            # Best effort, and never masking the original exception: the durable
            # PENDING marker is already the signal that settlement is incomplete,
            # so failing to enrich it must not change behaviour. The richer reason
            # is recorded by startup/runtime recovery, which runs in its own
            # session (nested writes here would collide with the fill's
            # transaction).
            try:
                async with self.database.session_factory() as session:
    ...
    async def _ensure_fill_settled(self, fill_id: str):
        ...
        Runs in SEPARATE short transactions per phase (ledger, projection+plan,
        marker) rather than one long one: the downstream refresh writes its own
        tables, so holding a single write transaction across it deadlocks against
        itself. Each phase is individually idempotent - the ledger is guarded on
        fill_id and the plan transition is a no-op when already applied - so
        re-entering after any interruption converges instead of duplicating.
        """
        ...
        from crypto_trader.order.settlement import SettlementOutcome

        return (
            SettlementOutcome(fill_id, STATE_COMPLETE, not already_accounted, already_accounted),
            already_accounted,
        )

    async def _settle_ledger_phase(self, fill, order):
        """Canonical ledger posting, reusing the fill-id idempotent path."""
        return await self._settle_fill(fill, record_metric=True)

    async def _settle_downstream_phase(self, fill, order, ledger_transaction_id):
        """Portfolio projection + TradePlan convergence (both idempotent)."""
        await self.portfolio.refresh(initial_balances=self._initial_balances or None)
        await self._converge_plan_for_fill(order)

    async def _converge_plan_for_fill(self, order) -> None:
        """Promote/close the plan implied by the now-factual position."""
        ...
        while True:
            ...
            for fill_id in fill_ids:
                ...
                if outcome.state == "COMPLETE":
                    ...
                    completed += 1
            if progressed == 0:
                # No forward progress: fail closed rather than spin.
                self.health.set(
                    "fill_settlement",
                    False,
