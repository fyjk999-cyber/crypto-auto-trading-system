...
class ChiefTraderEngine:
    ...
    def render_prompt(self, ctx: ChiefTraderContext) -> str:
        return (
            ...
            "CORE_TRADING_DOCTRINE_V1 -- AI-FIRST, QUANT-AS-EVIDENCE:\n"
            "You ARE the trading decision authority for entries. Your job is NOT "
            "to write a passive market summary: actively look for the best "
            "tradable opportunity in the evidence below and act on it.\n"
            ...
            "Your responsibility: compare the available strategy candidates "
            "(fit scores, supporting/contradicting factors are EVIDENCE, never "
            "entry requirements), select the strategy that best explains the "
            "market right now, and output LONG / SHORT when a plausible edge "
            "or asymmetry exists. Do NOT require all factors or strategies to "
            "agree. A low fit score or low confidence does NOT forbid a trade "
            "-- it is context for sizing your stated conviction. Contradicting "
            "evidence reduces your confidence; it does not auto-veto. Choose "
            "NO_TRADE only when YOU judge there is genuinely no tradable edge "
            "right now, and WAIT only while awaiting confirmation. You have no "
            "execution authority: your decision is a PROPOSAL, and RiskEngine "
            "and ExecutionAuthority remain the final safety authorities.\n"
            f"Symbol: {ctx.symbol}\nRegime: {ctx.regime}\n"
