...
class TradePlanORM(Base):
    ...
    entry_conditions_json: Mapped[list[Any] | None] = mapped_column(JSON)
    invalidation_conditions_json: Mapped[list[Any] | None] = mapped_column(JSON)
    reduce_conditions_json: Mapped[list[Any] | None] = mapped_column(JSON)
    exit_conditions_json: Mapped[list[Any] | None] = mapped_column(JSON)
    expected_holding_period: Mapped[str] = mapped_column(String(128), default="")
    max_holding_time_seconds: Mapped[float] = mapped_column(Float, nullable=False, default=86400.0)
    # --- Low-Risk V2 plan contract (migration 0024; all additive/nullable) ---
    ...
    based_on_state_version: Mapped[str | None] = mapped_column(String(64))
    base_exit_json: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    exit_approach: Mapped[str] = mapped_column(String(255), default="")
    adverse_trigger_json: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    thesis_invalidation: Mapped[str] = mapped_column(String(1000), default="")
    reassessment_rules_json: Mapped[list[Any] | None] = mapped_column(JSON)
    next_reassessment_json: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    expected_edge_bps: Mapped[float | None] = mapped_column(Float)
    expected_cost_bps: Mapped[float | None] = mapped_column(Float)
...
class TradeEpisodeORM(Base):
    ...
    risk_decision_ids_json: Mapped[list[Any] | None] = mapped_column(JSON)
...
class PositionLegORM(Base):
    ...
    average_entry_price: Mapped[Decimal | None] = mapped_column(ExactDecimal())
    closed_quantity: Mapped[Decimal | None] = mapped_column(ExactDecimal())
    realized_pnl: Mapped[Decimal | None] = mapped_column(ExactDecimal(), default=Decimal("0"))
    unrealized_pnl: Mapped[Decimal | None] = mapped_column(ExactDecimal(), default=Decimal("0"))
    fees: Mapped[Decimal | None] = mapped_column(ExactDecimal(), default=Decimal("0"))
    funding: Mapped[Decimal | None] = mapped_column(ExactDecimal(), default=Decimal("0"))
    terminal_reason: Mapped[str | None] = mapped_column(String(64))
    applied_fill_ids_json: Mapped[list[Any] | None] = mapped_column(JSON)
    opened_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    closed_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow)
...
class ScanSnapshotLabelORM(Base):
    ...
    alignment_error_seconds: Mapped[float | None] = mapped_column(Float)
    endpoint_policy: Mapped[str | None] = mapped_column(String(64))
    ...
    data_gap: Mapped[bool | None] = mapped_column(Boolean)
    factual_source: Mapped[str | None] = mapped_column(String(64))
    entry_price: Mapped[float | None] = mapped_column(Float)
    cost_version: Mapped[str | None] = mapped_column(String(64))
    cost_components_json: Mapped[dict[str, Any] | None] = mapped_column(JSON)
    maturation_status: Mapped[str | None] = mapped_column(String(32))
    usable_for_training: Mapped[bool | None] = mapped_column(Boolean, default=False)
    label_config_version: Mapped[str | None] = mapped_column(String(64))
    alignment_policy_version: Mapped[str | None] = mapped_column(String(64))
    raw_t0: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    aligned_bar_start: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    partial_start_bar: Mapped[bool | None] = mapped_column(Boolean)
    path_quality: Mapped[str | None] = mapped_column(String(32))
    endpoint_quality: Mapped[str | None] = mapped_column(String(32))
...
class OpportunityOutcomeMaturationORM(Base):
    ...
    endpoint_policy: Mapped[str] = mapped_column(String(40), default="CLOSED_BAR_END_LE_TARGET")
    final_bar_partial: Mapped[bool] = mapped_column(Boolean, default=False)
    data_gap: Mapped[bool] = mapped_column(Boolean, default=False)
    pages_fetched: Mapped[int] = mapped_column(Integer, default=0)
    maturation_status: Mapped[str] = mapped_column(String(32), default="UNKNOWN")
    usable_for_learning: Mapped[bool] = mapped_column(Boolean, default=False)
    quality_reason: Mapped[str | None] = mapped_column(String(64))
