...
class OrderManager:
    ...
    async def apply_fill(self, fill: Fill) -> tuple[Order, Fill, bool]:
        ...
        async with self.session_factory() as session:
            ...
            if existing is not None:
                # A durable fill is NOT necessarily a settled fill. Previously
                # this returned immediately, so a fill whose ledger posting /
                # portfolio projection / plan promotion was interrupted could
                # NEVER be completed - not by a redelivered event, not by
                # restart. The marker row is created here (idempotently) and the
                # caller is told the fill was already applied; settlement
                # completion is driven separately and idempotently.
